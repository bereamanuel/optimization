...
C=[-6  2  -3  0  0]' ; % Vector de costes
b=[2  4]'  ;      % Vector de recursos
A=[2  -1  2  1  0; 1  0  4  0  1]; % Matriz de restricciones
...
cNt=[-1  -1  -1]; % Condición inicial negativa para que entre en el bucle
...
while sum(cNt<-10*eps(1))>0
      ...
      CB=C(inB);  % Costo Básicos
      CN=C(inN);  % Costo No Básico
      ...
      [~,r]=max(cNt<-10*eps(1)); % Criterio de parada, el mayor antes de los negativos

      Ar=A(:,inN(r));           % Matriz cuyas columnas están asociadas a las No Básicas

      Artecho=B\Ar;              % Puede que existan algunos negativos

      Artecho(Artecho<-10*eps(1))=0;  % Esos valores negativos los elimino 

      [xs,s ]=min(btecho./Artecho); % División término a término matricialmente. Podríamos dividir
